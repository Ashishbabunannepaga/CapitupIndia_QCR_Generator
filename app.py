...
# Import the modern Google GenAI SDK safely
...
def process_pdf_document(file_bytes, file_name, prompt, key_manager, retries=6):
    ...
    for attempt in range(retries):
        ...
        try:
            ...
            # Defensive attribute parsing to prevent unexpected crash inside handler
            ...
            if error_code == 429:
                ...
                st.warning(f"🔄 Google Server Error ({error_code}) on {current_model}. Swapping to fallback model...")
                time.sleep(2)
            else:
                st.error(f"API Error on {file_name}: {getattr(e, 'message', str(e))}")
                time.sleep(5)
